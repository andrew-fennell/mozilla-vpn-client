...
if [[ "$MODE" == "pr" ]]; then
  ...
  qmllint -I nebula/ui -I src/ui $(gh pr view $PR_NUMBER --json files --jq '.files.[].path' | grep ".qml") 2> qml_lint_result.txt
else
  print N "\t Calling QML Lint staged git files"
  qmllint -I nebula/ui -I src/ui $(git diff --name-only --cached | grep ".qml") 2> qml_lint_result.txt
fi
